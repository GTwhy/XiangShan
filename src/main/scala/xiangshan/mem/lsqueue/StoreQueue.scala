...
class StoreQueue(implicit p: Parameters) extends XSModule
  ...
  if (env.EnableDifftest) {
    for (i <- 0 until StoreQueueSize) {
      ...
      difftest.io.coreid      := io.hartId
      difftest.io.index       := i.U
      difftest.io.valid       := allocated(i) && datavalid(i) && addrvalid(i) && committed(i)
      difftest.io.storeAddr   := debug_paddr(i)
      difftest.io.storeData   := debug_data(i)
      difftest.io.storeMask   := committed(i)
    }
  }
