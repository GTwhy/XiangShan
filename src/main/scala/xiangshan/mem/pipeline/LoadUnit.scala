  ...
  with HasDCacheParameters
{
  ...
  if (env.EnableDifftest) {
    ...
    // difftest.io.x           := RegNext(RegNext(RegNext(RegNext(RegNext(io.ldout.bits.uop.robIdx.value | (io.ldout.bits.debug.isMMIO << 16))))))
    difftest.io.valid       := valid
    difftest.io.paddr       := paddr
    difftest.io.loadData    := data
    difftest.io.loadMask    := diff_len
    difftest.io.x           := io.ldout.bits.uop.robIdx.value | (io.ldout.bits.debug.isMMIO << 16)
